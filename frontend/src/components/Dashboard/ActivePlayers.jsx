...
const PlayerAvatar = ({ src, online }) => (
  <div className="relative flex-shrink-0">
    <img 
      src={src || '/default-avatar.png'} // Asegúrate de tener un avatar por defecto
      alt="Pichanguero"
...
// Función helper para formatear el "time ago" (Last Seen)
...
const ActivePlayers = ({ currentUserEmail, className = "" }) => {
  ...
  // API_URL (Asumimos que la misma lógica del Dashboard)
  const API_URL = (import.meta.env.VITE_API_URL || "http://localhost:8000") + "/api";

  // Carga de Jugadores Activos
  useEffect(() => {
    const cargarJugadores = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Separar Online y Recientes
          const online = data.filter(j => j.estadoConexion === 'online');
          const recientes = data.filter(j => j.estadoConexion !== 'online');
          
          setJugadores({ online, recientes });
        }
      } catch (err) {
  ...
  const totalActivos = jugadores.online.length + jugadores.recientes.length;
